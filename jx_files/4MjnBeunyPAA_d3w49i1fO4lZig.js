;if(CloudflareApps.matchPage(CloudflareApps.installs['AlSIUBIrLtWo'].URLPatterns)){(function(){'use strict'
...
var options=window.CloudflareApps.installs['AlSIUBIrLtWo'].options
...
if("AlSIUBIrLtWo"==='preview')updateVisibility()}
function updateVisibility(){clearTimeout(visibilityTimeout)
var visible=true
var stored=false
try{stored=JSON.parse(store['cf-flashcard'])}catch(e){}
if("AlSIUBIrLtWo"==='preview'||!stored){visible=true}else{var timeframe=times[options.renew]
...
window.CloudflareApps.installs['AlSIUBIrLtWo'].scope={updateOptions:function updateOptions(nextOptions){clearTimeout(visibilityTimeout)
options=nextOptions
updateContent()
